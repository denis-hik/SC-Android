package ru.denishik.unity.other;

public class Config {
    public final static String SCOOTER_CONNECT_COMMAND = "scooter_connect";
    public final static String SCOOTER_GET_DATA_NAME_COMMAND = "scooter_get_name_data";
    public final static String SCOOTER_GET_DATA_PARAMS_COMMAND = "scooter_get_params_data";
    public final static String SCOOTER_SEND_DATA_PARAMS_COMMAND = "scooter_send_params_data";
    public final static String SCOOTER_ERROR_CONNECT = "scooter_error_connect";
    public final static String SCOOTER_ADV_PARAMS_SHOW = "scooter_adv_params_show";
    public final static String SCOOTER_ADV_PARAMS_DISABLE = "scooter_adv_params_disable";

    public final static String SCOOTER_STATUS_CONNECTED = "scooter_connected";
    public final static String SCOOTER_STATUS_LOADING = "scooter_loading";
    public final static String SCOOTER_STATUS_ERROR = "scooter_error";

    public final static String SCOOTER_LED = "scooter_led";
    public final static String SCOOTER_LED_CONNECT = "scooter_led_connected";
    public final static String SCOOTER_LED_TEXT = "scooter_led_text";
    public final static String SCOOTER_LED_ERROR = "scooter_led_err";
    public final static String SCOOTER_LED_DISCONNECT = "scooter_led_disconnected";
    public final static String SCOOTER_LED_RECONNECT = "scooter_led_reconnect";
}
